...
Widget allToolBuilder({required  List<ToolModel> list, required context}){
  return  Padding(
        ...
        child: GridView.builder(
          //shrinkWrap: true,
            ...
            itemBuilder: (BuildContext context, index) {
              return builtToolItem(list[index], context);
...
Widget builtToolItem(ToolModel model,context) {
                                ...
                                children: [
                                  Expanded(
                                    child: ElevatedButton(
                                      onPressed: () {
                                        ToolModel m = ToolModel.fromToolModel(model);
                                        int index = cubit.SearchInCartData(m.toolId);

                                        if (index != -1) {        //if item founded in cart
                                          int? q = AppCubit.cartDb[index].quantity +
                                              cubit.productQuantity;

                                          cubit.updateData(quantity: q, id: model.toolId);
                                          ...
                                          cubit.insertToDatabase(id: model.toolId, imageUrl: model.imageUrl, name: model.name, price:500, quantity: model.counter);
