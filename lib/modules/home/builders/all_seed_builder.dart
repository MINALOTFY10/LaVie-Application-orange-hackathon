...
Widget allSeedBuilder({required  List<SeedModel> list, required context}){
  return  Padding(
        ...
        child: GridView.builder(
          //shrinkWrap: true,
            ...
            itemBuilder: (BuildContext context, index) {
              return builtSeedItem(list[index], context);
...
Widget builtSeedItem(SeedModel model,context) {
  AppCubit cubit = AppCubit.get(context);
  return BlocConsumer<AppCubit, AppState>(
      listener: (context, state) {},
      builder: (context, state) {
        String constImage = 'https://lavie.orangedigitalcenteregypt.com/uploads/dcbbe466-416c-4797-a496-7e054495d47b.jpg';
        //model.name == 'test' ? model = object : model = model;
        return Card(
            elevation: 4,
                  ...
                  children: [
                    Expanded(
                      ...
                      child: Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.only(topRight: const Radius.circular(10),topLeft: Radius.circular(10)),
                                ...
                                children: [
                                  Expanded(
                                    child: ElevatedButton(
                                      onPressed: () {
                                        SeedModel m = SeedModel.fromSeedModel(model);
                                        int index = cubit.SearchInCartData(m.seedId);

                                        if (index != -1) {        //if item founded in cart
                                          int? q = AppCubit.cartDb[index].quantity +
                                              cubit.productQuantity;

                                          cubit.updateData(quantity: q, id: model.seedId);
                                          ...
                                          cubit.insertToDatabase(id: model.seedId, imageUrl: model.imageUrl, name: model.name, price:500, quantity: model.counter);
